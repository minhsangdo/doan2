...
5. "hoc_bong": Hỏi về học bổng, chính sách hỗ trợ sinh viên, học bổng khuyến khích, học bổng tân sinh viên, học bổng doanh nghiệp.
   Ví dụ: "Trường có học bổng gì?", "Học bổng cho tân sinh viên?", "Điều kiện nhận học bổng?"
6. "so_sanh": Yêu cầu so sánh về hai hoặc nhiều ngành học, chuyên ngành với nhau về điểm chuẩn, tổ hợp môn, tính chất ngành.
   Ví dụ: "So sánh ngành CNTT và KHKMT", "Ngành dược và nha khoa khác nhau như thế nào?"
7. "tu_van_ca_nhan": Hỏi hệ thống tư vấn ngành học nào phù hợp với bản thân dựa trên điểm số hoặc khối thi cá nhân (ví dụ: "dựa vào hồ sơ của tôi", "ngành nào phù hợp với em", "em được 22 điểm khối A00 thì học ngành gì").
8. "khac": Hỏi về những việc không liên quan đến tuyển sinh, hoặc các thông tin chung chung, chào hỏi mở đầu.
...
"""
    ...
    def classify(self, message: str) -> str:
        ...
        try:
            ...
            # Verify valid intent
            valid_intents = ["nganh_hoc", "diem_chuan", "phuong_thuc", "nhap_hoc", "hoc_bong", "so_sanh", "tu_van_ca_nhan", "khac"]
    ...
    def _heuristic_fallback(self, message: str) -> str:
        ...
        if any(w in msg for w in ["nhập học", "hồ sơ", "giấy tờ", "ngày nào", "hạn"]):
            return "nhap_hoc"
        if any(w in msg for w in ["học bổng", "hoc bong", "hỗ trợ sinh viên", "chính sách học bổng"]):
            return "hoc_bong"
